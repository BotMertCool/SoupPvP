...
public class ServerScoreboard implements BoardAdapter {
    ...
    public List<String> getLines(Player player) {
        ...
        arcane.add("&7&m----------------------");
        arcane.add("&4Kills: &f" + user.getKills());
        arcane.add("&4Deaths: &f" + user.getDeaths());
        arcane.add("&4Killstreek: &f" + user.getCurrentKillstreak());
        arcane.add("&4Credits &f" + user.getCredits());
        if(KitPvP.pearlcooldown.onCooldown(player)){
